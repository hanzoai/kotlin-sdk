...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw HanzoInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw HanzoInvalidDataException("`$name` is not set")
            is JsonNull -> throw HanzoInvalidDataException("`$name` is null")
            else -> throw HanzoInvalidDataException("`$name` is invalid, received $this")
        }
